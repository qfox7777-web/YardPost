...
function AdBanner({ slot="top", style={} }) {
  ...
  // For now shows a placeholder until AdSense is set up
  return (
    <div style={{
      background: "#F5F0E8",
      ...
      padding: "16px",
      textAlign: "center",
      minHeight: slot==="top" ? "90px" : "250px",
      ...style,
    }}>
      {/* PASTE YOUR ADSENSE SNIPPET HERE — replace this div with your <ins class="adsbygoogle"> tag */}
      <div style={{ fontFamily:"'DM Mono', monospace", fontSize:"10px", color:"#AAA", letterSpacing:"0.08em", textTransform:"uppercase" }}>
        Advertisement
      </div>
      <div style={{ fontFamily:"'DM Mono', monospace", fontSize:"9px", color:"#CCC", marginTop:"4px" }}>
        AdSense slot: {slot}
      </div>
    </div>
  );
...
function SaleCard({ sale, onClick, userLat, userLng }) {
  ...
  const dist = (userLat && sale.lat && sale.lng)
    ? getDistanceMiles(userLat, userLng, sale.lat, sale.lng).toFixed(1)
    : null;

  return (
    <div
      onClick={() => !isPast && onClick(sale)}
      style={{
        background:"#FFFDF7", border:"2px solid #2A2A2A", borderRadius:"4px",
        cursor: isPast?"default":"pointer", opacity: isPast?0.55:1,
        overflow:"hidden", transition:"transform 0.15s, box-shadow 0.15s",
        boxShadow:"4px 4px 0 #2A2A2A", display:"flex", flexDirection:"column",
      }}
    ...
    >
      <div style={{background:"#E8C547",padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"2px solid #2A2A2A"}}>
        <span style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:"11px",letterSpacing:"0.08em",textTransform:"uppercase"}}>
          {sale.city}, {sale.state} {dist && <span style={{fontWeight:400,color:"#555"}}>· {dist} mi away</span>}
        </span>
        <span style={{background:badge==="Today"?"#C0392B":badge==="Tomorrow"?"#2D7A5B":badge==="Past"?"#888":"#2A2A2A",color:"#fff",fontFamily:"'DM Mono',monospace",fontSize:"10px",padding:"2px 8px",borderRadius:"2px",fontWeight:600}}>
          {badge}
        </span>
      </div>
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#FFFDF7",border:"3px solid #2A2A2A",borderRadius:"4px",boxShadow:"8px 8px 0 #2A2A2A",maxWidth:"560px",width:"100%",maxHeight:"90vh",overflow:"auto"}}>
        ...
        <div style={{padding:"20px"}}>
          {/* Ad inside modal */}
          <AdBanner slot="modal" style={{marginBottom:"16px",minHeight:"80px"}} />
...
function PostForm({ onPost, onClose }) {
  ...
  const submit = async () => {
    ...
    setSaving(false);
    if(error) { setGeoError("Error saving: "+error.message); return; }
    onPost(data[0]);
    onClose();
  };
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#FFFDF7",border:"3px solid #2A2A2A",borderRadius:"4px",boxShadow:"8px 8px 0 #2A2A2A",maxWidth:"600px",width:"100%",maxHeight:"92vh",overflow:"auto"}}>
        ...
        <div style={{padding:"20px",display:"flex",flexDirection:"column",gap:"14px"}}>
          {geoError && <div style={{background:"#FFF0F0",border:"2px solid #C0392B",borderRadius:"3px",padding:"10px",fontFamily:"'DM Mono',monospace",fontSize:"11px",color:"#C0392B"}}>{geoError}</div>}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 80px 80px",gap:"10px"}}>
            ...
            <div>
              ...
              <input value={form.state} onChange={e=>set("state",e.target.value)} maxLength={2} style={inputStyle(false)} />
            </div>
            <div>
...
export default function App() {
  ...
  const getLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setLocating(false); alert("Couldn't get your location. Please allow location access."); }
    );
  };

  const clearLocation = () => { setUserLat(null); setUserLng(null); setLocationLabel(""); if(sortBy==="distance") setSortBy("date"); };
  ...
  const handlePost = (sale) => { setSales(s=>[sale,...s]); };
  ...
  return (
    <div style={{minHeight:"100vh",background:"#F5F0E8"}}>
      <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;800&family=Crimson+Text:ital,wght@0,400;0,600;1,400&family=DM+Mono:wght@400;500;600&display=swap" rel="stylesheet" />
      ...
      <header style={{background:"#2A2A2A",borderBottom:"4px solid #E8C547",padding:"0 24px",position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:"1100px",margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 0"}}>
          ...
          <button onClick={()=>setShowForm(true)} style={{background:"#E8C547",border:"2px solid #E8C547",borderRadius:"3px",boxShadow:"3px 3px 0 #E8C54766",padding:"9px 18px",fontFamily:"'DM Mono',monospace",fontSize:"11px",fontWeight:600,cursor:"pointer",letterSpacing:"0.07em",textTransform:"uppercase",color:"#1A1A1A"}}>
            + Post a Sale
          </button>
        </div>
      </header>
      ...
      <div style={{maxWidth:"1100px",margin:"16px auto 0",padding:"0 24px"}}>
        <AdBanner slot="leaderboard" style={{minHeight:"90px"}} />
      </div>
      ...
      <div style={{background:"#E8C547",borderBottom:"3px solid #2A2A2A",padding:"32px 24px 24px",marginTop:"12px"}}>
        <div style={{maxWidth:"1100px",margin:"0 auto"}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:"8px",maxWidth:"640px"}}>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by city, keyword, or category…"
              style={{flex:"1 1 260px",...inputBase}} />
            {!userLat ? (
              <button onClick={getLocation} disabled={locating} style={{background:"#2A2A2A",color:"#E8C547",border:"2px solid #2A2A2A",borderRadius:"3px",padding:"9px 14px",fontFamily:"'DM Mono',monospace",fontSize:"11px",cursor:locating?"not-allowed":"pointer",letterSpacing:"0.06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:"6px",background:"#2A2A2A",border:"2px solid #2A2A2A",borderRadius:"3px",padding:"0 12px"}}>
                <span style={{fontFamily:"'DM Mono',monospace",fontSize:"10px",color:"#E8C547"}}>📍 {locationLabel}</span>
                ...
                <button onClick={clearLocation} style={{background:"none",border:"none",color:"#888",cursor:"pointer",fontSize:"14px",padding:"0"}}>×</button>
              </div>
            )}
          </div>
        </div>
      ...
      <main style={{maxWidth:"1100px",margin:"0 auto",padding:"28px 24px"}}>
        <div style={{fontFamily:"'DM Mono',monospace",fontSize:"10px",color:"#888",marginBottom:"16px",letterSpacing:"0.06em"}}>
          {loading ? "Loading sales…" : `${filtered.length} sale${filtered.length!==1?"s":""} found${userLat?` within ${radiusMiles} miles`:""}${search?` · "${search}"`:""}`}
        </div>
        ...
        {loading ? (
          ...
          <div style={{textAlign:"center",padding:"60px 20px"}}>
            ...
            <div style={{fontFamily:"'Crimson Text',serif",fontSize:"16px",color:"#BBB"}}>Try a different search or post the first sale in your area!</div>
          </div>
        ) : (
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:"20px"}}>
            {itemsWithAds.map((item,i) =>
              item.type==="ad"
                ? <div key={item.key} style={{gridColumn:"1/-1"}}><AdBanner slot={`inline-${i}`} style={{minHeight:"90px"}} /></div>
      ...
      {/* Footer Ad */}
      <div style={{maxWidth:"1100px",margin:"0 auto 0",padding:"0 24px 32px"}}>
        <AdBanner slot="footer" style={{minHeight:"90px"}} />
      </div>
      ...
      <footer style={{borderTop:"2px solid #2A2A2A",padding:"20px 24px",textAlign:"center",background:"#2A2A2A"}}>
        ...
        <div style={{fontFamily:"'DM Mono',monospace",fontSize:"9px",color:"#666",marginTop:"4px",letterSpacing:"0.07em"}}>FREE · LOCAL · NO ADS*</div>
      </footer>
